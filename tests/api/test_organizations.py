...
async def test_create_organization_invalid(
    ...
    response = await client.post("/organizations/", json=payload, auth=temp_user.auth)
...
@pytest.mark.asyncio
async def test_create_organization_many(
    client: httpx.AsyncClient, temp_user: TestUser
) -> None:
    for _ in range(5):
        ...
        response = await client.post(
            "/organizations/", json=payload, auth=temp_user.auth
        )
        assert response.status_code == 201
