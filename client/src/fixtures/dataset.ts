...
export const getFakeDataSetFormData = (dataSetFormData: Partial<DatasetFormData>): DatasetFormData => {
    return {
        updateFrequency: dataSetFormData.updateFrequency || UPDATE_FREQUENCY.daily,
        title: dataSetFormData.title || "Mon jeu de donnée",
        contactEmails: dataSetFormData.contactEmails || [],
        ...
        firstPublishedAt: dataSetFormData.firstPublishedAt || new Date().toISOString(),
        lastPublishedAt: dataSetFormData.lastPublishedAt || new Date().toISOString(),
